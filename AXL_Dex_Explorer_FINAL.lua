...
║   ╚═╝  ╚═╝  ╚════╝     ╚═════╝  ╚════╝                              ║
║                                                                      ║
║        Classic Dex Explorer — مستكشف ديكس الكلاسيكي                 ║
║                      [ V 2.0.0 ]                                     ║
║                                                                      ║
║  © 2025 AXL  |  جميع الحقوق محفوظة  |  All Rights Reserved          ║
║                                                                      ║
║  يُمنع نسخ أو توزيع هذا الكود دون إذن مسبق من المطور AXL            ║
║  Unauthorized copying or distribution is strictly prohibited.        ║
║                                                                      ║
║  الميزات:  مستكشف الكائنات | الخصائص | محرر السكريبت | الريموتس    ║
╚══════════════════════════════════════════════════════════════════════╝
--]]

-- ═══════════════════════════════════════════════════════════════════════
--  شاشة التحميل بأسلوب Liquid Glass — مدة 5 ثواني حقيقية
--  AXL Liquid Glass Loader — Real 5 Second Loading Screen
-- ═══════════════════════════════════════════════════════════════════════
...
-- ───────────────────────────────────────────
--  🎨 ألوان Liquid Glass الأساسية
-- ───────────────────────────────────────────
local LG = {
    -- خلفية داكنة زجاجية
    BG_Dark       = Color3.fromRGB(8,   10,  20),
    BG_Glass      = Color3.fromRGB(15,  20,  40),

    -- تدرج أزرق سماوي مائع
    Cyan          = Color3.fromRGB(0,   220, 255),
    Blue          = Color3.fromRGB(30,  100, 255),
    Purple        = Color3.fromRGB(130,  50, 255),

    -- لمعان ونعومة
    WhiteGlass    = Color3.fromRGB(200, 230, 255),
    GlowSoft      = Color3.fromRGB(0,   180, 230),

    -- أخضر لحظة الاكتمال
    GreenOK       = Color3.fromRGB(0,   255, 160),
...
-- ───────────────────────────────────────────
--  بناء الـ ScreenGui الخاص بالتحميل
-- ───────────────────────────────────────────
local LoaderGui = Instance.new("ScreenGui")
LoaderGui.Name              = "AXL_LiquidGlass_Loader"
...
Background.BackgroundColor3 = LG.BG_Dark
Background.BorderSizePixel  = 0
Background.ZIndex           = 1
Background.Parent           = LoaderGui

-- ── طبقة زجاجية داخلية (shimmer) ──
local GlassLayer = Instance.new("Frame")
GlassLayer.Name                  = "GlassLayer"
GlassLayer.Size                  = UDim2.new(1, 0, 1, 0)
GlassLayer.BackgroundColor3      = LG.BG_Glass
GlassLayer.BackgroundTransparency = 0.7
GlassLayer.BorderSizePixel       = 0
GlassLayer.ZIndex                = 2
GlassLayer.Parent                = Background

-- ── دائرة ضوء خلفية (glow blob) ──
local GlowBlob = Instance.new("Frame")
GlowBlob.Name                   = "GlowBlob"
GlowBlob.Size                   = UDim2.new(0, 500, 0, 500)
GlowBlob.Position               = UDim2.new(0.5, -250, 0.5, -250)
GlowBlob.BackgroundColor3       = LG.Cyan
GlowBlob.BackgroundTransparency = 0.88
GlowBlob.BorderSizePixel        = 0
GlowBlob.ZIndex                 = 3
GlowBlob.Parent                 = Background
...
Card.BackgroundColor3       = LG.WhiteGlass
...
-- حد زجاجي لامع على البطاقة
local CardStroke = Instance.new("UIStroke", Card)
CardStroke.Color       = LG.Cyan
...
LogoText.TextColor3           = LG.White
LogoText.TextStrokeColor3     = LG.Cyan
LogoText.TextStrokeTransparency = 0.3
LogoText.ZIndex               = 11
LogoText.Parent               = Card

-- ── النص الفرعي ──
local SubText = Instance.new("TextLabel")
SubText.Name                  = "SubText"
SubText.Size                  = UDim2.new(1, -40, 0, 28)
SubText.Position              = UDim2.new(0, 20, 0, 100)
SubText.BackgroundTransparency = 1
SubText.Text                  = "Classic Dex Explorer  •  مستكشف ديكس"
SubText.Font                  = Enum.Font.Gotham
SubText.TextSize              = 15
SubText.TextColor3            = LG.WhiteGlass
SubText.TextTransparency      = 0.1
SubText.ZIndex                = 11
SubText.Parent                = Card

-- ── نص الحالة (يتغير أثناء التحميل) ──
...
BarBG.Position               = UDim2.new(0, 30, 0, 240)
BarBG.BackgroundColor3       = LG.BG_Glass
...
ProgressBar.BackgroundColor3      = LG.Cyan
...
-- تدرج لوني على شريط التحميل
local BarGradient = Instance.new("UIGradient", ProgressBar)
BarGradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0,   LG.Cyan),
    ColorSequenceKeypoint.new(0.5, LG.Blue),
    ColorSequenceKeypoint.new(1,   LG.Purple),
})

-- ── نسبة التحميل ──
local PercentLabel = Instance.new("TextLabel")
PercentLabel.Name                  = "PercentLabel"
PercentLabel.Size                  = UDim2.new(1, -60, 0, 24)
PercentLabel.Position              = UDim2.new(0, 30, 0, 255)
PercentLabel.BackgroundTransparency = 1
PercentLabel.Text                  = "0%"
PercentLabel.Font                  = Enum.Font.GothamBold
PercentLabel.TextSize              = 13
PercentLabel.TextColor3            = LG.WhiteGlass
...
CopyLabel.Position              = UDim2.new(0, 20, 0, 300)
CopyLabel.BackgroundTransparency = 1
CopyLabel.Text                  = "© 2025 AXL — جميع الحقوق محفوظة"
CopyLabel.Font                  = Enum.Font.Gotham
CopyLabel.TextSize              = 11
CopyLabel.TextColor3            = LG.WhiteGlass
CopyLabel.TextTransparency      = 0.4
CopyLabel.ZIndex                = 11
CopyLabel.Parent                = Card

-- ── نقاط التحميل المتحركة (Liquid Dots) ──
local DotsFrame = Instance.new("Frame")
DotsFrame.Name                    = "DotsFrame"
DotsFrame.Size                    = UDim2.new(0, 60, 0, 20)
DotsFrame.Position                = UDim2.new(0, 20, 0, 165)
...
for i = 1, 3 do
    ...
    dot.Position               = UDim2.new(0, (i-1)*18, 0.5, -5)
    dot.BackgroundColor3       = LG.Cyan
...
-- ═══════════════════════════════════════════════════════════════════════
--  🚀 منطق التحميل الحقيقي — 5 ثواني
-- ═══════════════════════════════════════════════════════════════════════

-- رسائل الحالة بالعربي
local statusMessages = {
    [0]   = "جاري التهيئة...",
    [20]  = "تحميل واجهة DEX...",
    [40]  = "ربط المستكشف...",
    [60]  = "تفعيل الخصائص...",
    [80]  = "تجهيز محرر السكريبت...",
    [95]  = "اكتمل التحميل! ✓",
}

local LOAD_TIME   = 5     -- ثواني حقيقية
local START_TICK  = tick()

-- تحريك النقاط بشكل مائع
local dotPhase = 0
RunService.Heartbeat:Connect(function(dt)
    dotPhase = dotPhase + dt * 3
    for i, dot in ipairs(dots) do
        local offset = math.sin(dotPhase + (i - 1) * 1.2)
        local alpha  = (offset + 1) / 2
        dot.BackgroundTransparency = 1 - (alpha * 0.8 + 0.1)
        dot.Size = UDim2.new(0, 8 + alpha * 4, 0, 8 + alpha * 4)
        dot.Position = UDim2.new(0, (i-1)*18 - alpha*2, 0.5, -4 - alpha*2)
    end
end)

-- تحريك بقعة الضوء الخلفية
local blobPhase = 0
RunService.Heartbeat:Connect(function(dt)
    blobPhase = blobPhase + dt * 0.5
    local bx = 0.5 + math.sin(blobPhase)       * 0.15
    local by = 0.5 + math.cos(blobPhase * 0.7) * 0.10
    GlowBlob.Position = UDim2.new(bx, -250, by, -250)
end)

-- تحديث شريط التحميل كل فريم
local loaderDone = false
RunService.Heartbeat:Connect(function()
    if loaderDone then return end
    ...
    PercentLabel.Text = pctInt .. "%"

    -- تحديث رسالة الحالة
    for threshold, msg in pairs(statusMessages) do
        if pctInt >= threshold then
            StatusText.Text = msg
        end
    end

    -- تغيير اللون لما يوصل 95%
    if pctInt >= 95 then
        ProgressBar.BackgroundColor3 = LG.GreenOK
        BarGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0,   LG.GreenOK),
            ColorSequenceKeypoint.new(1,   LG.Cyan),
        })
        StatusText.TextColor3 = LG.GreenOK
        CardStroke.Color      = LG.GreenOK
    end

    -- اكتمل التحميل
    if elapsed >= LOAD_TIME and not loaderDone then
        loaderDone = true

        task.wait(0.4) -- لحظة صغيرة بعد الاكتمال

        -- اخفاء ناعم مع تأثير Fade
        TweenService:Create(
            LoaderGui,
        ...
        TweenService:Create(
            ...
            { BackgroundTransparency = 1, Size = UDim2.new(0, 400, 0, 300) }
        ):Play()

        task.wait(0.9)

        -- إزالة الـ Loader تماماً وتشغيل DEX
        LoaderGui:Destroy()
        CreateGui()  -- ← تشغيل DEX الحقيقي
    end
end)

-- ═══════════════════════════════════════════════════════════════════════
--  📦 كود DEX الأصلي الكامل
--  Original DEX Code — Untouched
-- ═══════════════════════════════════════════════════════════════════════
...
║   © 2025 AXL — Classic Dex Explorer V2.0.0                          ║
