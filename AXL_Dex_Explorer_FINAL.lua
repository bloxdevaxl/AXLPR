-- AXL GARDEN - نسخة محسنة
-- شكراً للذكاء الاصطناعي على المساعدة
-- تقدر تاخذ السورس وتحسنه
-- مسموح للكل يستخدمه
...
local DEFAULT_THEME = {
	...
	Panel2 = Color3.fromRGB(14, 14, 20),
	Panel3 = Color3.fromRGB(22, 22, 32),
	Text = Color3.fromRGB(235, 240, 255),
	Muted = Color3.fromRGB(110, 120, 150),
	Accent = Color3.fromRGB(120, 140, 255),
	Accent2 = Color3.fromRGB(80, 90, 220),
	Danger = Color3.fromRGB(255, 60, 80),
	Stroke = Color3.fromRGB(40, 44, 70),
	DarkText = Color3.fromRGB(2, 2, 4),
...
function KrassUI.new(config)
	...
	local blur = nil
	local blurTarget = config.BlurSize or 24
	...
	local rootStroke = stroke(theme.Stroke, 1.5, 0.05)
...
if not Net then
    warn("[AXL GARDEN] ما لقينا موديول الشبكة — صح الجيم؟")
    return
end
...
local S = {
    ...
    autoPlant = false, plantSpacing = 4, plantSeed = "الأحسن عندك",
...
local function pickPlantTool()
    if S.plantSeed ~= "الأحسن عندك" and S.plantSeed ~= "" then
...
local function stepSell()       -- sell-only mode (when Auto-Harvest is off)
    ...
    local n = sellAllNow()
    if n > 0 then warn("[AXL GARDEN] باع " .. n .. " غرضة") end
end
...
local function sendWebhook(isTest)
    if not httpRequest then warn("[AXL GARDEN] المشغل ما عنده دعم HTTP"); return false end
    if not string.match(S.webhookUrl or "", "^https?://") then warn("[AXL GARDEN] حط رابط ويبهوك صحيح"); return false end
    local payload = { username = "AXL GARDEN", embeds = { {
        title = "🌱 تقرير المزرعة — " .. LocalPlayer.Name, color = 5763719,
        fields = {
            { name = "💰 فلوس", value = fmt(getSheckles()), inline = true },
            { name = "🪙 توكن",   value = fmt(getTokens()),   inline = true },
            { name = "🌾 الأرض",     value = tostring((myPlot() and myPlot().Name) or "?"), inline = true },
            { name = "📊 إحصائيات الجلسة",  value = string.format("شرى %d · زرع %d · حصد %d · باع %d (+%s)",
                Stats.bought, Stats.planted, Stats.harvested, Stats.sold, fmt(Stats.earned)), inline = false },
            { name = "✨ إضافيات",   value = string.format("مرشات %d · ري %d · روّض %d · فتح %d · سرق %d",
                Stats.sprinklers, Stats.watered, Stats.tamed, Stats.opened, Stats.stolen), inline = false },
            { name = "⏱️ وقت التشغيل",   value = hms(os.clock() - Stats.startAt), inline = true },
        }, footer = { text = "AXL GARDEN · بالتوفيق" },
    } } }
    ...
    if isTest then warn("[AXL GARDEN] " .. (good and "تقرير تجريبي أُرسل ✅" or ("فشل الإرسال (" .. tostring(code) .. ")"))) end
    return good
end
...
local ui = KrassUI.new({
    Name = "AXL GARDEN",
    Subtitle = "كامل أوتوماتيك | فل أوتو",
    Theme = "Black",
...
local farmTab = ui:Tab("🌱 المزرعة")
local boostsTab = ui:Tab("⚡ البوستات")
local petsTab = ui:Tab("🐾 الحيوانات")
local openTab = ui:Tab("🥚 البيض والصناديق")
local shopTab = ui:Tab("🛒 المحل")
local stealTab = ui:Tab("🌙 السرقة")
local miscTab = ui:Tab("⚙️ متفرقات")
local settingsTab = ui:Tab("🔧 الإعدادات")

-- ---- FARM ----
local secStatus = farmTab:Section("الحالة")
local plotLabel = secStatus:Label("الأرض: ...")
local cashLabel = secStatus:Label("فلوس: ...")
local statLabel = secStatus:Label("—")

local secMaster = farmTab:Section("أوتو-فارم (رئيسي)")
secMaster:Toggle("🔁 أوتو-فارم (شري+زرع+حصاد+بيع+توسيع)", false, function(v) S.autoFarm = v end)
secMaster:Toggle("🌿 أوتو-توسيع الحديقة", false, function(v) S.autoExpand = v end)
secMaster:Toggle("📅 أوتو-عروض اليومية", false, function(v) S.autoDaily = v end)

local secBuy = farmTab:Section("شري البذور")
secBuy:Dropdown("البذور الي تريد تشريها", SEED_NAMES, {}, function(sel) pickMulti(sel, S.buySeeds) end)
secBuy:Toggle("🛒 أوتو-شري المحدد", false, function(v) S.autoBuy = v end)
secBuy:Slider("وقت الشراء (ثانية)", 5, 1, 30, function(v) S.buyInterval = v end)
secBuy:Slider("أكثر شراء / بذرة", 8, 1, 50, function(v) S.buyPerTick = v end)

local secPlant = farmTab:Section("زرع / حصاد / بيع")
local plantOpts = { "الأحسن عندك" }; for _, n in ipairs(SEED_NAMES) do plantOpts[#plantOpts + 1] = n end
secPlant:Dropdown("البذرة الي تريد تزرعها", plantOpts, "الأحسن عندك", function(v) S.plantSeed = v end)
secPlant:Toggle("🌱 أوتو-زرع (ملي الأرض)", false, function(v) S.autoPlant = v end)
secPlant:Slider("مسافة الزرع", 4, 2, 10, function(v) S.plantSpacing = v end)
secPlant:Toggle("🍎 أوتو-حصاد الفواكه الناضجة", false, function(v) S.autoHarvest = v end)
secPlant:Slider("سرعة الحصاد (ث/فاكهة)", 0.01, 0, 0.2, function(v) S.harvestDelay = v end)
secPlant:Toggle("💰 أوتو-بيع (يبيع لما يتملا الكيس)", false, function(v) S.autoSell = v end)
secPlant:Slider("وقت البيع (ثانية)", 15, 3, 120, function(v) S.sellInterval = v end)
secPlant:Toggle("🪴 أوتو-تأصيص النباتات", false, function(v) S.autoPot = v end)

-- ---- BOOSTS ----
local secSpr = boostsTab:Section("المرشات والماي")
secSpr:Toggle("💧 أوتو-حط المرشات", false, function(v) S.autoSprinkler = v end)
secSpr:Slider("وقت المرشة (ثانية)", 30, 10, 120, function(v) S.sprinklerInterval = v end)
secSpr:Toggle("🚿 أوتو-ري بالمرشة", false, function(v) S.autoWater = v end)
secSpr:Slider("وقت الري (ثانية)", 8, 2, 60, function(v) S.waterInterval = v end)

local secSkill = boostsTab:Section("نقاط المهارة")
secSkill:Dropdown("الإحصائيات الي تريد ترفعها", { "BaseSpeed", "BaseJump", "ShovelPower", "MaxBackpack" }, {}, function(sel) pickMulti(sel, S.skillStats) end)
secSkill:Toggle("⭐ أوتو-صرف نقاط المهارة", false, function(v) S.autoSkill = v end)

-- ---- PETS ----
local secPet = petsTab:Section("الحيوانات")
secPet:Toggle("🐾 أوتو-تجهيز الحيوانات", false, function(v) S.autoEquipPets = v end)
secPet:Toggle("➕ أوتو-شري مكان للحيوانات", false, function(v) S.autoPetSlot = v end)
secPet:Toggle("🦊 أوتو-شري حيوانات العالم", false, function(v) S.autoBuyPets = v end)
secPet:Slider("أكثر سعر للحيوان (شيكل)", 25000, 1000, 1000000, function(v) S.maxPetPrice = v end)
secPet:Toggle("🚀 انتقل للحيوان (لازم للشراء)", true, function(v) S.petTeleport = v end)
secPet:Slider("وقت شراء الحيوان (ثانية)", 5, 2, 60, function(v) S.petBuyInterval = v end)

local secPetSell = petsTab:Section("بيع الحيوانات")
secPetSell:Dropdown("الحيوانات الي تريد تبيعها", ownedPetNames(), {}, function(sel) pickMulti(sel, S.sellPets) end)
secPetSell:Toggle("💸 أوتو-بيع الحيوانات المحددة", false, function(v) S.autoSellPets = v end)

-- ---- EGGS & CRATES ----
local secOpen = openTab:Section("فتح تلقائي")
secOpen:Toggle("🥚 أوتو-فتح البيض", false, function(v) S.autoEgg = v end)
secOpen:Toggle("📦 أوتو-فتح الصناديق", false, function(v) S.autoCrate = v end)
secOpen:Toggle("🌿 أوتو-فتح أكياس البذور", false, function(v) S.autoPack = v end)
secOpen:Slider("وقت الفتح (ثانية)", 4, 1, 30, function(v) S.openInterval = v end)
local secOpenInfo = openTab:Section("معلومات")
secOpenInfo:Label("يفتح كل شي عندك بكل")
secOpenInfo:Label("فئة. التأكيد تلقائي.")

-- ---- SHOP ----
local secShop = shopTab:Section("محل الأدوات")
secShop:Dropdown("الأدوات الي تريد تشريها", GEAR_NAMES, {}, function(sel) pickMulti(sel, S.gearBuy) end)
secShop:Toggle("⚒️ أوتو-شري الأدوات المحددة", false, function(v) S.autoGear = v end)
secShop:Slider("وقت شراء الأدوات (ثانية)", 10, 2, 60, function(v) S.gearInterval = v end)

-- ---- STEAL ----
local secSteal = stealTab:Section("أوتو-سرقة (بس بالليل)")
secSteal:Toggle("🌙 أوتو-سرقة فواكه الثانيين", false, function(v) S.autoSteal = v end)
secSteal:Toggle("🚀 انتقل للفاكهة (لازم للسرقة)", true, function(v) S.stealTeleport = v end)
secSteal:Toggle("🏠 ارجع للبيت بعد كل فاكهة", true, function(v) S.stealReturnBase = v end)
secSteal:Slider("سرعة السرقة (0=فوري)", 0.05, 0, 1, function(v) S.stealDelay = v end)
local secStealInfo = stealTab:Section("معلومات")
secStealInfo:Label("بس بالليل · انتقل للفاكهة وسرقها")
secStealInfo:Label("ثم ارجع للبيت تحسبها.")

-- ---- MISC ----
local secMail = miscTab:Section("البريد والهدايا")
secMail:Toggle("📬 أوتو-استلام البريد", false, function(v) S.autoMail = v end)
secMail:Toggle("🎁 أوتو-قبول الهدايا", false, function(v) S.autoAcceptGift = v end)

local secHop = miscTab:Section("الجلسة")
secHop:Toggle("🛡️ ضد الكيك (ما ينطرد)", true, function(v) S.antiAfk = v end)
secHop:Toggle("🔄 أوتو-تغيير السيرفر", false, function(v) S.autoHop = v end)
secHop:Slider("غير سيرفر كل (دقيقة، 0=إيقاف)", 0, 0, 120, function(v) S.hopInterval = v * 60 end)

local secCode = miscTab:Section("الأكواد")
secCode:Textbox("فعّل كود", "اكتب الكود هنا", function(text)
    if text and text ~= "" then
        local ok, res = fire("Settings.SubmitCode", text)
        warn("[AXL GARDEN] " .. ((ok and res == true) and ("تم تفعيل الكود: " .. text) or ("كود غلط: " .. text)))
    end
end)
secCode:Toggle("📋 أوتو-تفعيل قائمة الأكواد", false, function(v) S.autoCodes = v end)

-- ---- SETTINGS ----
local secPerf = settingsTab:Section("الأداء والواجهة")
secPerf:Toggle("🚀 تحسين FPS (جرافيك منخفض)", false, function(v) S.fpsBoost = v; applyFpsBoost(v) end)
secPerf:Button("❌ إغلاق الهب (يوقف كلشي)", function() S.killed = true; pcall(function() ui:Destroy() end) end)

local secWeb = settingsTab:Section("ويبهوك الديسكورد")
secWeb:Textbox("رابط الويبهوك", "https://discord.com/api/webhooks/...", function(t) S.webhookUrl = t or "" end)
secWeb:Toggle("📊 تفعيل التقارير", false, function(v) S.webhookEnabled = v end)
secWeb:Slider("وقت التقرير (دقيقة)", 5, 1, 60, function(v) S.webhookInterval = v * 60 end)
secWeb:Button("📤 إرسال تقرير تجريبي", function() task.spawn(function() sendWebhook(true) end) end)

local secInfo = settingsTab:Section("معلومات")
secInfo:Label("AXL GARDEN · كامل أوتوماتيك")
secInfo:Label("مفتاح: Ctrl يساري يفتح/يغلق الواجهة")
...
task.spawn(function()
    while not S.killed do
        local p = myPlot()
        pcall(function() plotLabel:Set("الأرض: " .. (p and p.Name or "?")) end)
        pcall(function() cashLabel:Set(string.format("فلوس: %s · توكن: %s", fmt(getSheckles()), fmt(getTokens()))) end)
        pcall(function() statLabel:Set(string.format("شرى %d · زرع %d · حصد %d · باع %d (+%s)",
...
warn("[AXL GARDEN] تم التحميل بنجاح · " .. #SEED_NAMES .. " بذرة · " .. #GEAR_NAMES .. " أداة")
print("[AXL GARDEN] تم تحميل الهب بنجاح - استمتع يا صديقي!")
